                        ...
                        <div class="table-body mb-4">
                            @if(count($products) == 0 )
                                <div class="flex items-center justify-center p-20">
                                    <h1 class="font-[Nunito] text-[24px]">No Products available!!</h1>
                                </div>
                            @endif

                            @foreach($products as $product)
                                <div aria-keyshortcuts="{{ $product -> id}}" class="flex items-center justify-between h-[50px] px-8 mt-2 font-[Nunito] rounded-[10px] hover:bg-white hover:shadow-neutral-400 cursor-pointer">
                                    <div class="row basis-[100%]">{{$product -> name}}</div>
                                    <div class="row basis-[80%]">{{$product -> category}}</div>
                                    <div class="row basis-[80%]">{{$product -> quantity}} Pcs</div>
                                    <div class="row basis-[80%]">{{$product -> price}} DH</div>
                                    <div class="row basis-[80%]">{{$product -> quantity * $product -> price  }} DH</div>
                                    <div class="row relative basis-[20%] items-end">
                                        <span class="action" id="action" onclick="openMenu(this, {{$product->id}})">
                                        ...
                                        <div class="hidden" id="action-menu">
                                            <div  class="absolute z-50 bg-white p-2 flex flex-col gap-2">
                                                ...
                                                <span>
                                                    <form action="{{route('products.destroy', parameters: $product->id)}}" method="POST" >
                                                        @csrf
                                                        @method('DELETE')
            ...
            <div class="add-products absolute top-0 right-0 h-screen  w-[400px]  py-16 px-10 bg-white overflow-y-scroll">
                <div class="new-product z-10 text-[#030229]">
                    <div class="head flex items-center gap-8">
                        ...
                        <h1 class="font-bold text-[18px]">Add New Product</h1>
                    </div>
