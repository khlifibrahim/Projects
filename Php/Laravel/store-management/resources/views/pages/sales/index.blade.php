    ...
    {{-- <div class="flex justify-between"> --}}
        <div class="products h-full w-full p-16 font-[Nunito]">
            <div class="flex items-center justify-between">
                <h1 class="font-bold text-2xl font-[Nunito]">product list</h1>
            ...
            <div class="products-list">
                <div class="flex items-center justify-between h-[50px]  px-8 mt-10 font-[Nunito] text-[#030229] rounded-[10px]">
                    <div class="row basis-[100%]">Product</div>
                    <div class="row basis-[80%]">Quantity</div>
                    <div class="row basis-[80%]">Price</div>
                    <div class="row basis-[80%]">Paid ?</div>
                ...
                <div class="table-body mb-4">
                    @if(count($sales) == 0 )
                        <div class="flex items-center justify-center p-20">
                            <h1 class="font-[Nunito] text-[24px]">No Sales available!!</h1>
                        </div>
                    @endif
                    ...
                    @foreach($sales as $sale)
                        <div class="flex items-center justify-between h-[50px] px-8 mt-2 font-[Nunito] rounded-[10px] hover:bg-white hover:shadow-neutral-400 cursor-pointer">
                            <div class="row basis-[100%]">{{$sale -> product -> name }}</div>
                            <div class="row basis-[80%]">{{$sale -> quantity}} Pcs</div>
                            <div class="row basis-[80%]">{{$sale -> price}} DH</div>
                            <div class="row basis-[80%]">
                                <span class="px-4 py-2 {{ $sale -> debt === true ? 'bg-[#E71D36]/30 text-[#E71D36]' : 'bg-green-400/20 text-green-400'}}  font-semibold rounded-full">{{ $sale -> debt === true ? 'Not yet' : 'Paid' }}</span>
                            </div>
                            <div class="row basis-[80%]">{{$sale -> date  }}</div>
                            <div class="row relative basis-[20%] items-end">
                                <span class="action" id="action" onclick="openMenu(this, {{$sale->id}})">
                                ...
                                <div class="hidden" id="action-menu">
                                    <div  class="absolute z-50 bg-white p-2 flex flex-col gap-2">
                                        ...
                                        <span>
                                            <form action="{{route('sales.destroy', parameters: $sale->id)}}" method="POST" >
                                                @csrf
                                                @method('DELETE')
            ...
            <div class="add-products absolute top-0 right-0 h-screen  w-[400px]  py-16 px-10 bg-white overflow-y-scroll">
                <div class="new-product z-10 text-[#030229]">
                    <div class="head flex items-center gap-8">
                        ...
                        <h1 class="font-bold text-[18px]">New Sale</h1>
                    </div>
...
@push('scripts')
    <script>
        ...
        const sales = document.getElementById('product-form');
